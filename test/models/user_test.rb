...
class UserTest < ActiveSupport::TestCase
  ...
  test "users name can not be too long" do
    @user.name = '#' * 21
    assert_not @user.valid?
  end

  test "users email can not be too long" do
    @user.name = '#' * 44 + '@163.com'
    assert_not @user.valid?
  end

  test "users email should in valid format" do
    valid_mails = %w[users@example.com USER@foo.COM A_US-ER@foo.bar.org first.last@foo.jp alice+bob@baz.cn]
  ...
  test "users email should be saved as lowercase" do
  ...
  test 'users passwd should be present' do
    @user.password = @user.password_confirmation = ''
    assert_not @user.valid?
  end

  test 'users passwd should have minimum length' do
